function [Gp, Gpp, nerr, lr2f ,li2f, r2, i2] = ...
    DI_for_simulation(dx,dy, NXY, mask, fms, NRO, NPH, filter, phi_array)
...
% May-7-2021===RKW===Modification for the multi-task reconstruction
...
% filter
phi_array = filter2(h, phi_array);

% get fundamental comoponents
% r2 = r24(:,:,2); i2 = i24(:,:,2);
r2 = real(phi_array); i2 = imag(phi_array);
...
%     u=ones(size(b))*std(b);
    ...
    % estimated displacement form the G*
    %D(II(ii),JJ(ii))=XY;
